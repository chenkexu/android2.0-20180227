 ...
 * 省市
 */
public class ProvinceCityCountyDialog extends Dialog{
    //点击监听
    ...
    public ProvinceCityCountyDialog(Context context) {
        super(context);
        init(context);
    }

    protected ProvinceCityCountyDialog(Context context, boolean cancelable, OnCancelListener cancelListener) {
        super(context, cancelable, cancelListener);
        init(context);
    }

    public ProvinceCityCountyDialog(Context context, int themeResId) {
    ...
    public void setUpData(final List<Area> areas) {
        this.mAreas = areas;
        adapter = new MyAdapter(mContext,0,areas);
        mViewArea.setAdapter(adapter);
        mViewArea.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                changeListener.onChangeListener(areas.get(position),position);
